...
const 压缩所有图片 = async (imageAssets, 压缩质量, 压缩级别, 文件格式) => {
    let compressionPromises = imageAssets.map(asset => {
        let outputPath = 生成压缩目标文件名(asset.path, 压缩质量, 文件格式);
        return compressImageFile(asset.path, outputPath, 压缩质量, 压缩级别, 文件格式);
    });
...
export const 重命名 = (asset) => ({
    label: '重命名',
    click: async () => {
        const newName = await showInputDialogPromise('重命名', '请输入新的文件名');
        if (newName) {
            // Implementation of renaming the file
            console.log(`Renaming ${asset.path} to ${newName}`);
        } else {
            console.log('用户取消操作');
        }
    }
