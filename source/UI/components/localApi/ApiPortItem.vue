<template>
  <li @click.right="() => 打开搜索面板(
    ...
    <everythingPortInputter :host="api.host" v-model="api.port" v-if="api.type === 'everything'">
    </everythingPortInputter>
    <input v-if="api.type !== 'everything'" v-model="api.port" type="number" min="1" max="65535"
      :style="{ color: isEnabled ? 'green' : 'red' }">
  </li>
</template>
<script setup>
