import { 反向柯里化 } from '../../base/useEcma/forFunctions/forCurrying.js'
/**
 * 图像变换模块 - 处理图像的几何和空间变换
 */
...
export const 创建变换预设 = ({ 尺寸, 选项 } = {}) => {
    // 返回一个函数，它接受 Sharp 对象和可选的额外参数
    return (img, ...args) => {
        let finalSize = 尺寸;
        let finalOptions = 选项;
        
        // 如果 args 包含尺寸和选项，根据类型区分
        if (args.length > 0) {
            if (Array.isArray(args[0])) {
                finalSize = finalSize || args[0];
                if (typeof args[1] === 'object' && args[1] !== null) {
                    finalOptions = { ...finalOptions, ...args[1] };
                }
            } else if (typeof args[0] === 'object' && args[0] !== null) {
                finalOptions = { ...finalOptions, ...args[0] };
            }
        }
        
        // 确保 finalSize 和 finalOptions 有效
        if (!finalSize) {
            console.error('创建变换预设：缺少必要的尺寸参数');
            return img; // 或者抛出错误
        }
        finalOptions = finalOptions || {};

        return 调整大小(img, finalSize, finalOptions);
    };
}; 