  ...
  router.post(`${prefix}/test`, {
    ...
    try {
      ...
      const requestHeaders = {
        // Preserve some headers from the original meta request?
        // 'cookie': ctx.get('cookie'), // Forward cookies?
        // 'authorization': ctx.get('authorization'), // Forward auth header?
        'Content-Type': 'application/json',
        'X-Meta-Test-Request': 'true', // Header to indicate this is a meta test
        ...headers
      };
      ...
      // Send the test request using fetch
      const response = await fetch(testUrl, {
        method: method.toUpperCase(),
        headers: requestHeaders,
        body: (method.toUpperCase() !== 'GET' && method.toUpperCase() !== 'HEAD') ? JSON.stringify(bodyParams) : undefined,
      ...
      try {
          ...
          // If parsing fails, try getting text content
          try{
             responseBody = await response.text();
          } catch(textError) {
      ...
      // Return test result
      ctx.status = 200; // Meta request itself succeeded
      ctx.body = {
        testRequest: {
            url: testUrl,
            method: method.toUpperCase(),
      ...
      console.error('Error during meta test execution:', error);
      ctx.status = 500;
      ctx.body = {
        error: `Meta test execution failed: ${error.message}`,
        stack: process.env.NODE_ENV === 'production' ? undefined : error.stack
      };
    }
  });

  console.log(`Meta API endpoints registered under ${prefix}.`);
  return router;
} 