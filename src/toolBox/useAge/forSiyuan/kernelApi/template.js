...
const sendTemplateRequest = async (endpoint, data, options = {}) => {
  try {
    const apiUrl = getSiyuanApiUrl(`/api/template/${endpoint}`, options.host);
    const token = options.token || localStorage.getItem('token');
    const headers = createAuthHeaders({ token: token, scheme: 'Token' });
    headers['Content-Type'] = 'application/json';

    const response = await forFetchWithTimeout(apiUrl, {
      method: 'POST',
      headers: headers,
    ...
    return handleSiyuanApiError(err, `模板 ${endpoint} 操作`);
  }
};
